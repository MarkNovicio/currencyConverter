...
const initiallyDisplayedCurrencies = ["USD", "EUR", "GBP", "JPY", "RUB", "AUD"];
let baseCurrency;
...
function addCurrencyListClick(event) {
  const clickedListItem = event.target.closest("li"); //identify which element was clicked
  ...
  if (event.target.classList.contains("close")) {
    ...
    if (parentNode.classList.contains("base-currency")) {
      ...
      if (newBaseCurrencyLI) {
        //if newBaseCurrencyLI is NOT empty
...
function setNewBaseCurrency(newBaseCurrencyLI) {
  ...
  currenciesList.querySelectorAll(".currency").forEach((currencyLI) => {
    const currencyRate = currencies.find(
      (c) => c.abbreviation === currencyLI.id
    ).rate;
    const exchangeRate =
...
//populateAddCurrencyList() iterates through the currencies array of objects
//each iteration targets the "add-currency-list" class and adds the currency's
//abbreviation, flag, and name using the insertAdjacentHTML method
  ...
  for (let i = 0; i < currencies.length; i++) {
    addCurrencyList.insertAdjacentHTML(
      ...
      `<li data-currency=${currencies[i].abbreviation}>
        <img src=${currencies[i].flagURL} class="flag" /><span>
        ${currencies[i].abbreviation} - ${currencies[i].name}</span>
        </li>`
    );
  }
}

//populateCurrenciesList() iterates through the countries in the initiallyDisplayedCurrencies array.
//Each country in the list is crosschecked by the Currencies array of objects.
//If the country abbreviation in the initiallyDisplayedCurrencies array matches the country
//abbreviation in the Currencies array of objects then a matched item in the initiallyDisplayedCurrencies
//array will be stored in the currency variable.  When there exists a value for the currency variable,
//the newCurrenciesListItem() function gets called and the currency value is passed in as an argument.
...
//The value of the currency  paramater is passed through the function.
//
function newCurrenciesListItem(currency) {
  if (currenciesList.childElementCount === 0) {
    console.log("currenciesList.childElementCount === 0");
    baseCurrency = currency.abbreviation;
    console.log(`baseCurrency1 ${baseCurrency}`);
  ...
  const inputValue = baseCurrencyAmount
    ...
    : "";
  console.log(baseCurrencyAmount * exchangeRate);
  console.log("input value" + baseCurrencyAmount * exchangeRate);
...
fetch(dataURL)
  ...
  .then((data) => {
    ...
    document.querySelector(".date").textContent = data.date
      .split("-")
      .reverse()
      .join("-"); //insert date into the date class element
    data.rates["EUR"] = 1;
